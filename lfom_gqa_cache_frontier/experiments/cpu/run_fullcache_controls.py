...
OUT=Path('/mnt/data/lfom_gqa_distill_fullcache_controls_2seed'); OUT.mkdir(exist_ok=True)
...
for seed in range(2):
    ...
    t0=time.time(); teacher=train_teacher(seed,rng,120); ce,ac=evalm(teacher,rng,20); rows.append(dict(seed=seed,method='MHA_teacher',kv=4,cache=1.0,ce=ce,acc=ac,trainable=sum(p.numel() for p in teacher.parameters()),time=time.time()-t0)); print(seed,'teacher',ce,ac,flush=True)
    for kv,prefix in [(4,'MHA'),(2,'GQA'),(1,'MQA')]:
        if kv != 4:
            b=compress(teacher,kv,'none'); ce,ac=evalm(b,rng,20); rows.append(dict(seed=seed,method=prefix,kv=kv,cache=kv/n_heads,ce=ce,acc=ac,trainable=0,time=0)); print(seed,prefix,ce,ac,flush=True)
        for res in ['mlp','lfom']:
            t0=time.time(); m=compress(teacher,kv,res); train_res(m,teacher,rng,70,res); ce,ac=evalm(m,rng,20); rows.append(dict(seed=seed,method=prefix+'_'+res.upper(),kv=kv,cache=kv/n_heads,ce=ce,acc=ac,trainable=count_train(m),time=time.time()-t0)); print(seed,prefix,res,ce,ac,count_train(m),flush=True)
...
for seed,g in raw.groupby('seed'):
    ...
    for lf in ['GQA_LFOM','MQA_LFOM']:
        for base in ['MHA_teacher','MHA_MLP','MHA_LFOM','GQA','GQA_MLP','MQA','MQA_MLP']:
...
# headline
headline=summary[summary.method.isin(['MHA_teacher','MHA_MLP','MHA_LFOM','GQA','GQA_LFOM','MQA','MQA_LFOM','GQA_MLP','MQA_MLP'])]
...
with zipfile.ZipFile('/mnt/data/lfom_gqa_distill_fullcache_controls_2seed_package.zip','w',zipfile.ZIP_DEFLATED) as z:
